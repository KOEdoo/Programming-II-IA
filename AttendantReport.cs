...
{
    public partial class AttendantReport : Form
    {
        ...
        public void LoadReport()
        {
            ...
            DataTable reportTable = Attendant.GetDailyReport(this.id);
            foreach (DataRow row in reportTable.Rows)
            {
                dataGridViewReport.Rows.Add(row[0].ToString(), row[1].ToString(), row[2].ToString());
            }
        }
